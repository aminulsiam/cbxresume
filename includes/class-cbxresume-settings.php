	...
	class CBXResumeSettings {
		...
		function callback_multicheck( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				...
				$html .= sprintf( '<p class="multicheck_field"><!--<span class="multicheck_field_handle"><i class="dashicons dashicons-move"></i></span>--><label for="wpuf-%1$s[%2$s][%3$s]">',
					$args['section'], $args['id'], $key );
				$html .= sprintf( '<input type="hidden" name="%1$s[%2$s][]" value="" />', $args['section'],
					$args['id'] );
